...
///
/// It is two rows in five, so it is not a footnote. The temptation is
...
fn reading(corpus: &Corpus) -> String {
    ...
    format!(
        "## Reading it\n\nDuckDB over the files in place, without downloading the repository:\n\n```sql\nCREATE SECRET (TYPE huggingface, PROVIDER credential_chain);\n\nSELECT host, crawl_delay_ms, sitemaps\nFROM read_parquet('hf://datasets/{repo}/data/**/*.parquet')\nWHERE status = 200 AND crawl_delay_ms IS NOT NULL\nLIMIT 20;\n```\n\nThe files are around 128 MB and a day folder holds one day of them, so a query that names a day reads a day. Rows are in fetch completion order, which means `fetched_at_ms` is the column whose row group statistics are worth pushing a filter down to, and `host` is not: a run walks the domain ranking rather than the alphabet, so hosts are spread across every file.\n\nOne host can appear more than once, though as the corpus stands it rarely does: counted on 2026-09-06 there were 26,987,785 rows over 26,863,085 distinct hosts, so 0.46 percent of the rows are a second answer for a host already in the corpus. Four days earlier it was 23.1 percent. That was not history. Every one of those rows was fetched inside a single day by two runs whose slices of the domain ranking overlapped, because nothing in the crawler stopped a run asking a host an earlier run had already answered. Something does now, the files carrying most of the repeats were taken back out, and the hosts whose only answer was in one of them were asked again. The removal is recorded under `retractions/` in the meta repository, with every removed file named and the digest it had.\n\nWhat is left is small enough to ignore for most work and it is still there, so take the newest `fetched_at_ms` per host if what you want is one current answer each:\n\n```sql\nSELECT host, status, crawl_delay_ms, allows_us\nFROM read_parquet('hf://datasets/{repo}/data/**/*.parquet')\nQUALIFY row_number() OVER (PARTITION BY host ORDER BY fetched_at_ms DESC) = 1;\n```\n\nThat reads every file, so narrow the path to the days you want if you do not need all of it.\n\n"
    )
}
