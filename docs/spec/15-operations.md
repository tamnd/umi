...
What that looks like on a box nobody has set up yet is worth writing down, because the default is quietly slow rather than broken. Ubuntu points `/etc/resolv.conf` at `127.0.0.53`, the systemd-resolved stub, which is a small forwarding cache in front of whatever the provider gave the machine. Measured on server3 against two thousand cold names: 133 answers a second with 64 lookups outstanding, 138 with 512, and past 256 outstanding the failures climb steeply because queries are queueing at the stub rather than waiting on the network. A crawler reads a failed lookup as a host that would not answer, so an overloaded stub does not only slow the crawl down, it fabricates dead hosts and writes them into the corpus. The same box with `unbound` in front of it answered 244 a second at 256 outstanding and 276 at 1024, with the failure count staying at the floor the list itself sets. End to end, the bulk robots prefetch over twenty thousand cold hosts went from 150 hosts a second and 19.3 percent unanswered to 223 a second and 14.3 percent, which is more throughput and eight hundred more hosts whose rules we actually read.

Pointing the crawler at a resolver is an explicit setting and not something it works out for itself. `UMI_DNS_SERVERS` takes a comma separated list of addresses and overrides the platform configuration for the crawler alone. It is deliberately not automatic: the stub is also what carries a box's VPN, its split horizon names and its search domains, and a crawler that quietly went around it would resolve differently from everything else on the machine, which is the kind of difference that takes a day to find.

Installing `unbound` also does not route anything to it, which is worth saying because the box looks correct afterwards and is not. Ubuntu leaves `/etc/resolv.conf` pointing at the systemd-resolved stub and resolved keeps forwarding to whatever the provider handed the machine, so a freshly installed recursive resolver sits on 127.0.0.1 answering nothing while every lookup still goes out to the provider. Both fleet boxes were in that state for a while and it is invisible from the crawl: the rate looks like the rate, and the only sign is that `unbound-control stats` shows a query count that has nothing to do with what the crawler is doing. `UMI_DNS_SERVERS=127.0.0.1` is what actually connects the two and the fleet sets it on every run.

Installing `unbound` is not the end of it, because the default sizing is for a household and not for a crawler, and getting it wrong looks exactly like a web full of dead hosts. Two settings decide it. `num-queries-per-thread` is how many queries one thread will hold at once and `outgoing-range` is how many sockets it has to ask upstream with, both per thread. When the first is reached unbound drops the query rather than queueing it, the crawler waits out its six second robots budget, and a host that was never asked is written into the corpus as a host that did not answer. On server3 at 1024 fetches in flight, 1,022,897 queries out of 5,302,968 were dropped that way. On server2, which had two threads rather than four, 12,991,989 out of 20,725,145 were, which is why that box was reporting more than sixty percent of the deep tail as silent when the real figure is nothing like it. `unbound-control stats_noreset | grep requestlist.exceeded` is the counter, it should be zero, and it is the first thing to look at when the unanswered share does not match what the same list gives somewhere else.

The sizing rule is that the resolver has to be able to hold more queries at once than the crawler can have fetches outstanding, with room left for the second lookup a redirect needs. The fleet runs four threads with `num-queries-per-thread: 4096` and `outgoing-range: 8192`, which is sixteen thousand queries in flight against a fetch window of a few thousand. Every outgoing slot is a file descriptor, so four threads at 8192 is 32768 of them and the unit needs `LimitNOFILE` well past that, which is a systemd drop in and not a line in the unbound config. The crawler needs its own limit raised for the same reason and for its own sockets, and a default login shell on Ubuntu still gets 1024.
