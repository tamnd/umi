...
A robots.txt gets 6 seconds rather than 30. It is a different kind of request from a page and the numbers say so. Measured over 500 hosts off a real seed list, 64 at a time, on the 30 second budget: the median robots.txt arrives in 695 ms, the ninetieth percentile is 10004 ms, which is the connect timeout to the millisecond, and the ninety ninth is the full 30 seconds. So more than a tenth of hosts never complete a handshake for the file, and each one holds a slot in the fetch window for ten seconds waiting. Seventy three percent of all the time that batch spent was the part of each fetch past three seconds. On a 6 second budget the same 500 hosts cost a mean of 1434 ms instead of 3749 and the batch finished in 13.4 seconds instead of 28.3, at the cost of about one percent of the useful answers. Those hosts are then disallowed for a day under doc 07.4, which is a real cost and is why the number is 6 and not 2. The connect timeout is unchanged because it lives in the connector and is per client, and a shorter total deadline makes it moot anyway.
