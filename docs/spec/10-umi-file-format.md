...
It also carries `observed_secs`, which is how long the URL has been watched for, summed over the intervals we actually served. That column is the denominator of doc 09's change rate estimator and `change_count` is only the numerator, so a spill that dropped it would hand the row back with the estimate reset and the site would fall back to the default schedule. A domain that gets evicted often is exactly the one that can least afford to be refetched on a guess, so the column has to survive the round trip.
