...
Step 1 checks one thing doc 10 cannot. The chunk checksums say the segment is undamaged and they cannot say it is the right segment, because a file whose blocks all check out is still only self consistent. The one fact about it recorded anywhere else is the row count the state ledger took when the segment was sealed, so that is compared against the row count the conversion produced, and a disagreement stops the publish. There is no way to tell from inside the publisher which of the two is wrong, and the alternative is a manifest carrying a row count that nothing else in the system agrees with, followed by 12.7 deleting the only copy that could have settled it.
...
Statistics matter more than they look. A consumer filtering on `fetched_at_ms` or on `host` should read three row groups, not the whole file, and that only works if the min and max are written and the writer did not sort rows in a way that makes every row group's range cover everything. Doc 10.5 considered a 4096 row reorder window that groups by host, which would have incidentally made the host statistics useful, and measured it out. So the host min and max on a row group cover most of the alphabet and a consumer filtering on host gets no help from them. The `fetched_at_ms` statistics are the ones that work, because rows are written in fetch completion order and that is already sorted.
...
**`open-index/umi-frontier-<YYYY>w<WW>-<NN>`** is doc 08.6's backlog: one row per URL we know about and have not fetched, with the scheduling state that decides when we would. It is here for two reasons and the first one is ours. A hundred billion known URLs is terabytes of ledger and the fleet has 342 GB of local disk, so the backlog cannot live on the boxes doing the crawling, and object storage that anyone can also read costs us nothing over object storage only we can read. The second reason is everyone else's. A ranked list of URLs that a crawler has canonicalised, deduplicated and scheduled is the thing that is hardest to build from scratch and that nobody publishes, and doc 04's fetcher protocol gets a bulk work transport out of it for free: a community fetcher downloads a file and knows what to fetch without asking us for anything.

Sliced by week and slice like pages and receipts rather than held in one repository like robots, because it is the largest family the project has. Nothing looks a spill up by repository name, so the slicing costs nothing: a coordinator finds a domain through the per domain pointer doc 08.6 keeps locally, and everyone else reads the manifest. Rows are in `(pld_id, host_id, url_key)` order, which is the local ledger's primary key, so a reader after one site reads the row groups whose statistics cover it and skips the file.

Nothing in this family has been fetched, so nothing in it has been checked against the origin's `robots.txt`, and the dataset card says so where a reader will see it. A URL in the backlog is a URL we intend to ask about under RFC 9309, not a URL anybody has permission for.
...
The publishing keys live in `umi-meta` under `keys/publishing/<key_id>.json`, where `key_id` is the first eight bytes of the blake3 of the raw public key, in hex. Each file holds the role, the key as `ed25519:<hex>`, its own key id, and the time it was added. A publisher writes its key there before it signs anything, because a manifest signed by a key nobody outside that machine can find is a manifest nobody outside that machine can check. Writing it first also means the failure shows up at the start of a crawl rather than after a day of fetching.

A manifest does not say which key signed it. A verifier reads every publishing key in the directory and accepts the manifest if any of them verifies the signature. That is deliberate, because a key identifier in the manifest is a field an attacker gets to fill in, and the version without it lets rotation cost nothing: an old key stays in the directory and old manifests keep verifying under it. Ed25519 verification is microseconds and the directory holds one entry per key ever used, so trying all of them is cheaper than the request that fetched the manifest.
...
"In this order" is not decoration. Condition 1 is answerable from the listing alone, and it is answered before the read back in condition 2 is paid for, because an object that is already the wrong size does not become the right size by being downloaded. There is a second reason. An upload cut off by a dropped connection leaves a short object, and a ranged read of a short object comes back short, which the hub client reports as a transport failure. Doc 14.9 maps a transport failure to exit 5 and a script reads exit 5 as worth retrying, when the honest answer for a copy that did not arrive whole is exit 6. Checking the size first turns that case into the verification failure it is.
...
The rule above was written as though a deletion could not happen. One has, so the rule needs a second sentence rather than a stronger first one. On 2026-09-06 forty one robots files were removed from `open-index/umi-robots`, holding 4,809,400 rows of which 4,173,280 were a second answer for a host the corpus already covered, because two runs whose slices of the domain ranking overlapped had asked the same hosts. The decision to remove them was the operator's and it cost 636,120 rows that were the only answer for their host. Those hosts were asked again straight afterwards, at 341.3 hosts per second with 4.6 percent of them not answering, so the cost was about half an hour and the rows that came back are fresher than the ones that went. What matters here is not that decision but what it taught us about the mechanism, which is that a project with no supported way to delete a file does not stop deletions happening. It gets them done by hand, and a file removed by hand leaves the day manifest naming something that is not there, leaves the chain pointing at a digest nothing computes to, and leaves nothing at all saying who did it or why. A reader checking that repository finds damage they cannot tell from an attack.

So there is a command, `umi retract`, and the promise changes shape. We still never rewrite a published file, and the exclusion list is still the mechanism for a correction, and a retraction is still the wrong answer to almost every problem that looks like it wants one. What we now promise instead is that if a file does come out, it comes out visibly. The deletions and the rewritten day manifests go in a single commit, so there is no moment at which a reader can fetch a manifest naming a file the same commit removed. The chain is relinked from the first changed day to the end of the repository, because 12.5 gives each day the digest of the day before it and rewriting one day orphans every day after it. A record naming every removed file with the digest it had, the row and byte counts, the days rewritten and the reason in words is published to `umi-meta` under `retractions/` before the deletion, so the explanation exists before the thing it explains. Nothing in that repairs a digest somebody recorded before the retraction, and nothing can. The record is the whole mitigation: a reader whose check now fails can find out what happened rather than concluding they were served tampered data.
...
The robots card carries two sections the others do not, because it has a column readers get wrong and a name they can rely on. The first says what a zero status is: roughly two rows in five carry one, a host with no `robots.txt` answers 404 rather than nothing, and a zero means the request came back empty because the name did not resolve or the connection failed. Left unsaid, a reader counts several hundred million hosts as having no rules when what we have is no answer, and reads `allows_us = 0` on those rows as the host refusing them rather than as RFC 9309 applied to our own failure. The second is a worked DuckDB query against the repository. It is on this card alone because every other family splits by week and slice, so its repository name carries a date the card generator is not given, and a snippet naming the wrong repository is worse than none.

That second section also says how many rows are a second answer for a host already in the corpus, and how to take one row per host. It was 23.1 percent when it was first counted, on 2026-09-02 over 18,515,689 rows and 14,244,161 distinct hosts, and almost none of it was history: every one of those rows was fetched inside a single day by two runs whose slices of the domain ranking overlapped. `umi robots --known` stopped new runs adding to it and the retraction described in 12.8 took most of what was already there back out, so counted again on 2026-09-06 over 26,987,785 rows and 26,863,085 distinct hosts it is 0.46 percent. The card carries the number and the query either way, because the query is the right way to read this corpus whether or not the repeats are down to a rounding error, and a reader who trusts a number on a card instead of counting is going to be wrong eventually.
