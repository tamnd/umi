...
The rule above was written as though a deletion could not happen. One has, so the rule needs a second sentence rather than a stronger first one. On 2026-09-02 forty one robots files were removed from `open-index/umi-robots`, holding 4,809,400 rows of which 4,173,280 were a second answer for a host the corpus already covered, because two runs whose slices of the domain ranking overlapped had asked the same hosts. The decision to remove them was the operator's and it cost 636,120 rows that were the only answer for their host, all of which were refetched afterwards. What matters here is not that decision but what it taught us about the mechanism, which is that a project with no supported way to delete a file does not stop deletions happening. It gets them done by hand, and a file removed by hand leaves the day manifest naming something that is not there, leaves the chain pointing at a digest nothing computes to, and leaves nothing at all saying who did it or why. A reader checking that repository finds damage they cannot tell from an attack.
...
That second section also says how many rows are a second answer for a host already in the corpus, and how to take one row per host. It was 23.1 percent when it was first counted, on 2026-09-02 over 18,515,689 rows and 14,244,161 distinct hosts, and almost none of it was history: every one of those rows was fetched inside a single day by two runs whose slices of the domain ranking overlapped. `umi robots --known` stops new runs adding to it, and the retraction described in 12.8 took most of what was already there back out. The card carries the number and the query either way, because the query is the right way to read this corpus whether or not the repeats are down to a rounding error, and a reader who trusts a number on a card instead of counting is going to be wrong eventually.
