...
That second section also says how many rows are a second answer for a host already in the corpus, and how to take one row per host. It is 23.1 percent as the corpus stands, counted on 2026-09-02 over 18,515,689 rows and 14,244,161 distinct hosts, and almost none of it is history: every one of those rows was fetched inside a single day by two runs whose slices of the domain ranking overlapped. `umi robots --known` stops new runs adding to it. The rows already published stay, because section 12.2 says a published file is never rewritten and never deleted, so the card carries the number and the query rather than a promise that the files will be cleaned up.
