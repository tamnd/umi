...
There is a golden corpus: 10000 documents committed to the repository covering the shapes that break extractors, including single page applications, AMP variants, `<noscript>` heavy pages, RTL scripts, CJK without spaces, HTML emitted by Word, tables used for layout, documents with three `<body>` tags, and 200 KB of nested `<div>`. Every build asserts byte identical output against the recorded digests. Any diff is either an intentional major version bump with the golden digests updated in the same commit, or a bug. There is no third case, and this test is what makes the determinism rule in 11.1 real rather than aspirational.
