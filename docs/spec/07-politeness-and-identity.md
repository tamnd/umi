...
Fetching the file is never allowed to sit in a page slot, and on a broad crawl that is the single largest thing between us and doc 16's gate 3.1. A crawl at depth two meets a host nobody has seen on most of its pages, so most page slots would otherwise open a connection for robots.txt, wait for it, and only then ask for the page they were given. Measured on server3 that was 1739 ms of a 2526 ms page, and it stays that high rather than fading after the first minute, because link discovery keeps finding hosts. Two things keep it off the page. The file is fetched when the lease is taken rather than when it is dispatched, which gives it whatever runway the lease spends in the queue, and a slot that reaches a lease whose file has still not landed puts that lease at the back of the queue and takes one it can fetch now. The lease is not dropped and the file is not asked for twice, since a host has one fetch in flight at a time whoever started it. Politeness is unchanged either way: the prefetch takes the host's next slot exactly as a lease would, so the page that follows takes the one after it and the origin sees the same spacing it always did.
