...
`--patience` is the setting that decides the rate, and it took three bands to see that. Ranks 10 to 18 million came back 35.6 percent silent and ran at 275.9 hosts a second, ranks 18 to 26 million were 55.9 percent silent and ran at 155.3, and a pass over 636,120 hosts already known to answer was 4.2 percent silent and ran at 341.3. Nothing else about those runs differed enough to explain the spread. A host that answers costs a lookup and one small request; a host that does not costs the whole wait, and twice over, because the second ask above pays it again. Raising `--concurrency` does not help, because the slots are not what is short. What is short is the seconds each dead host holds one for.

So the number is a trade rather than a safety margin, and it is the only knob here that buys rate by giving something up. Turning it down finishes a band sooner and loses the hosts that would have answered slowly. How much it loses is a question about the web and not about the code, so it is measured on real bands rather than guessed at, and the default is what the measurement settled on. A crawl of pages keeps the fetcher's longer default, because a page is worth waiting for and there is one robots.txt per host either way.
