...
`stats` is answered from a maintained counters row rather than by counting. The first version counted, which was five table scans, 97 ms over 200000 urls on server3, and linear, so a hundred million urls would have been 78 seconds a call. The crawl loop asks for it on every idle tick, to tell pending urls waiting their turn apart from a frontier that is actually empty, and doc 14.3's progress line asks every five seconds, and neither of those calls can be dropped. So there is one row of nine counters and a trigger on every table that changes them, which puts the arithmetic inside the writing transaction by construction: a crash commits the rows and the counts together or neither, and a write path added later cannot forget to update them. The measured cost is 0.02 ms a call, flat, against about 20 percent off admission throughput, which is a trade worth making at 70000 candidates a second against a target of 12500.

The counters can still be wrong, because a file can be edited by hand with the `sqlite3` shell or restored from half a backup, so the scan stays available on demand and reports which counter disagrees and by how much. `umi check` is where it surfaces.
